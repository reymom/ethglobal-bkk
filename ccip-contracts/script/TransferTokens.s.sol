...
contract TransferTokens is Script {
    ...
    address receiver;
    address rerouter = 0xe92b7Fc7E9ab829ae582975C9989B78AF0eFF461;
    ...
    function run() external {
        ...
        uint256 destinationChainId = HelperUtils.getUintFromJson(
            vm, configPath, string.concat(".transferRemoteChains.", HelperUtils.uintToStr(block.chainid))
        );
        ...
        if (routerContract.isChainSupported(destinationChainSelector)) {
            ...
            RerouterParams memory rerouterparams =
                RerouterParams({destinationChainSelector: destinationChainSelector, receiver: msg.sender});
            extraData = abi.encode(rerouterparams); // We will send the final destination chain to the rerouter in the data field
            receiver = rerouter; // rerouter contract on Sepolia
            destinationChainSelector = 16015286601757825753; // Sepolia chain selector
        }
