...
/// @title - A rerouter contract to reroute tokens bridged to Sepolia to another destination chain
/// @dev - This example shows how to recover tokens in case of revert
contract Rerouter is CCIPReceiver, OwnerIsCreator {
    ...
    address s_reroutedToken; // The address in Sepolia of the token getting rerouted
    bytes32 private s_lastReceivedMessageId; // Store the last received messageId.
    uint256 private s_lastReceivedTokenAmount; // Store the last received amount.
    RerouterParams private s_lastReceivedParams; // Store the last received text.
    ...
    constructor(address _router, address _link, address _reroutedToken) CCIPReceiver(_router) {
        s_linkToken = IERC20(_link);
        s_reroutedToken = _reroutedToken;
    }
    ...
    function sendMessagePayLINK(uint64 _destinationChainSelector, address _receiver, address _token, uint256 _amount)
        internal
        ...
        returns (bytes32 messageId)
    {
        ...
        emit MessageSent(messageId, _destinationChainSelector, _receiver, _token, _amount, address(s_linkToken), fees);
     ...
     * @return destinationChainSelector The selector of the destination chain of the last received CCIP message.
     * @return receiver The receiver in the destination chain of the tokensreceived in the last CCIP message.
     * @return tokenAmount The amount of the token in the last CCIP received message.
     */
    function getLastReceivedMessageDetails()
        public
        view
        returns (bytes32 messageId, uint64 destinationChainSelector, address receiver, uint256 tokenAmount)
    {
        return (
            s_lastReceivedMessageId,
            s_lastReceivedParams.destinationChainSelector,
            s_lastReceivedParams.receiver,
        ...
        onlyAllowlisted(any2EvmMessage.sourceChainSelector, abi.decode(any2EvmMessage.sender, (address))) // Make sure the source chain and sender are allowlisted
    {
        ...
        try this.processMessage(any2EvmMessage) {
            // If processMessage succeeds, we send the tokens to the final destination
            sendMessagePayLINK(
                s_lastReceivedParams.destinationChainSelector,
                s_lastReceivedParams.receiver,
                s_reroutedToken,
    ...
    function _ccipReceive(Client.Any2EVMMessage memory any2EvmMessage) internal override {
        ...
        s_lastReceivedParams = abi.decode(any2EvmMessage.data, (RerouterParams)); // abi-decoding of the sent parameters
        ...
        returns (Client.EVM2AnyMessage memory)
    {
        ...
        Client.EVM2AnyMessage memory evm2AnyMessage = Client.EVM2AnyMessage({
            ...
            data: abi.encode(), // empty field (no params neded to be sent)
